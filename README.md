...
- 🎯 Entry points to execute your program using `python -m <moudroandela>` or `$ moudroandela` with basic CLI argument parsing.
...
---
# moudroandela Flask Application

Awesome moudroandela created by matejkratochvil
...
```bash
git clone https://github.com/matejkratochvil/moudroandela moudroandela
cd moudroandela
make install
```
...
```bash
moudroandela create-db   # run once
moudroandela populate-db  # run once (optional)
moudroandela add-user -u admin -p 1234  # ads a user
moudroandela run
```
