...
original_author="matejkratochvil"
original_name="moudroandela"
original_urlname="moudroandela"
original_description="Awesome moudroandela created by matejkratochvil" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/moudroandela "${name}"
...
then
    ...
    echo "then run 'moudroandela' or 'python -m moudroandela'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'moudroandela' or 'python -m moudroandela' or 'poetry run moudroandela'"
fi
